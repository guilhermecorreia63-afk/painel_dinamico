...
def inicializar_banco():
    ...
    try:
        executar_batch(statements)
    except Exception as e:
        st.error(f"Erro ao inicializar tabelas: {e}")

# Inicializa as tabelas do banco no arranque do app
inicializar_banco()
...
# --- PROCESSAMENTO DE HISTÓRICO PRECISO (USANDO BANCO DE DADOS) ---
def verificar_e_notificar_mudancas():
    df_atual = st.session_state.get('df_pedidos_cache', pd.DataFrame())
    if df_atual.empty:
        return

    # Lê o histórico do banco de dados
    try:
        df_historico = executar_query("SELECT * FROM historico_pedidos")
    except Exception:
        ...
        try:
            ...
            if etapa_atual == 'Separar Estoque':
                if pedido_id not in df_historico['Pedido'].values:
                    ...
                    nova_linha = pd.DataFrame([{
                        ...
                        "DataEntradaFaturar": np.nan, "DataFaturamento": np.nan
                    }])
            ...
            # REGRA 2: Pedidos em Faturar
                ...
                if pedido_id not in df_historico['Pedido'].values:
                    ...
                    nova_linha = pd.DataFrame([{
                        "Pedido": pedido_id, "DataEntradaSeparacao": ts_separacao,
                        "DataEntradaFaturar": ts_faturar, "DataFaturamento": np.nan
                    }])
                    df_historico = pd.concat([df_historico, nova_linha], ignore_index=True)
                    houve_alteracao = True
                else:
            ...
            # REGRA 3: Pedidos Faturados
                ...
                if data_fat == hoje_omie or data_alt == hoje_omie:
                    ...
                    # Fallback para Entrada Faturar (usar data_alt/hora_alt se for pelo menos 1 minuto anterior ao faturamento)
                    ts_faturar = parse_and_adjust_datetime(data_alt, hora_alt)
                    if ts_faturar and ts_faturamento:
                        try:
                            diff_temp = pd.to_datetime(ts_faturamento) - pd.to_datetime(ts_faturar)
                            if diff_temp < pd.Timedelta(minutes=1):
                                ts_faturar = np.nan
                        except Exception:
                            ts_faturar = np.nan
                    else:
                    ...
                    if pedido_id in df_historico['Pedido'].values:
                        ...
                        nova_linha = pd.DataFrame([{
                            "Pedido": pedido_id, "DataEntradaSeparacao": ts_separacao,
                            "DataEntradaFaturar": ts_faturar, "DataFaturamento": ts_faturamento
                        }])
                        df_historico = pd.concat([df_historico, nova_linha], ignore_index=True)
                        houve_alteracao = True
...
with st.spinner("Carregando e processando pedidos do Omie..."):
    ...
    # Snapshot diário de resumo e detalhado
    if st.session_state.last_daily_snapshot_date < current_date:
        if not st.session_state.df_pedidos_cache.empty:
            ...
            # 1. Resumo
            statements_diarios.append((
                "INSERT OR REPLACE INTO historico_diario_resumo (Data, TotalPendentes, TotalFaturados) VALUES (?, ?, ?)",
                (current_date.strftime("%Y-%m-%d"), int(total_pendentes_snapshot), int(total_faturados_snapshot))
            ))
            
            # 2. Detalhes
            df_salvar = st.session_state.df_pedidos_cache.replace({np.nan: None})
            ...
            for _, row in df_salvar.iterrows():
                statements_diarios.append((
                    ...
                    (
                        ...
                        row.get('Hora Faturamento'),
                        row.get('Data Alteração'),
                        row.get('Hora Alteração'),
                        row.get('Data Inclusao'),
                        row.get('Hora Inclusao'),
            ...
            try:
                executar_batch(statements_diarios)
                st.toast(f"Snapshots diários salvos no banco de dados!", icon="📸")
                st.session_state.last_daily_snapshot_date = current_date
                st.rerun()
            except Exception as e:
                st.toast(f"Erro ao salvar snapshot diário: {e}", icon="⚠️")
    ...
    if not df_filtrado.empty:
        ...
        with st.container(border=True):
            ...
            if t_800 <= hora_atual <= t_1030:
                st.success("🟢 **HORÁRIO DOS CORREIOS (08:00 - 10:30):** Preferência activa para envios via Correios.")
            elif t_1030 < hora_atual < t_1400:
                st.info("⏳ **HORÁRIO J&T & TOTAL EXPRESS (10:30 - 14:00):** Preferência activa para J&T e Total Express.")
...
with aba_historico:
    from historico_turso import renderizar_aba_historico
